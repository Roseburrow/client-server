...
public class Client
{
    ...
    private void DetermineRequestToSend(StreamWriter sw)
    {
        if (location == null)
        {
    ...
    private void ReadResponse(string response)
    {
        ...
        Regex whois_NameResponse = new Regex(@"(.*)\r\n");
        Regex whois_LocationResponse = new Regex(@"^OK\r\n$");

        Regex http_NameResponse = new Regex(@"^HTTP/([^ ]+) 200 OK\r\nContent-Type: "
                                           + @"text/plain\r\n\r\n(.*)\r\n$");
        Regex http_LocationResponse = new Regex(@"^HTTP/([^ ]+) 200 OK\r\nContent-Type: "
                                          + @"text/plain\r\n\r\n$");

        Regex whois_NotFound = new Regex(@"^ERROR: no entries found\r\n$");
        Regex http_NotFound = new Regex(@"^HTTP/([^ ]+) 404 Not Found\r\nContent-Type: text/plain\r\n\r\n$$");

        if (whois_NotFound.IsMatch(response) || http_NotFound.IsMatch(response))
        {
            Console.WriteLine("ERROR: no entries found");
        }
        else if (http_NameResponse.IsMatch(response))
        {
            recievedLocation = http_NameResponse.Match(response).Groups[2].Value;
            Console.WriteLine(String.Format("{0} is {1}", name, recievedLocation));
        }
        else if (http_LocationResponse.IsMatch(response))
        {
            Console.WriteLine(String.Format("{0} location changed to be {1}", name, location));
        }
        else if (whois_LocationResponse.IsMatch(response) && location != null)
        {
            Console.WriteLine(String.Format("{0} location changed to be {1}", name, location));
        }
        else if (whois_NameResponse.IsMatch(response))
        {
            recievedLocation = whois_NameResponse.Match(response).Groups[1].Value;
            Console.WriteLine(String.Format("{0} is {1}", name, recievedLocation));
        }
    }
}
